...
_Generated 2026-06-06T19:59:39.081577+00:00 — machine output of `make verify`. Each claim is credited only because its external test passed._

**Must-pass gate: ✅ GREEN** (77/77) · verified 83/88 · failing 0 · unverified 5
...
| ✅ ⭐ | Street Manager-style planned works produce schema-valid timed road closures | `test_streetworks_sane` | verified |
| ✅ ⭐ | Kerbside loading and medical handoff zones are schema-valid and in London | `test_kerbside_loading_zones_sane` | verified |
| ✅ ⭐ | TfL roadside Variable Message Sign data is normalized for live narration | `test_roadsigns_sane` | verified |
...
| ✅ ⭐ | Anticipatory method directionally improves over naive greedy dispatch in the research study | `test_beats_greedy_directionally` | verified |
| ✅ ⭐ | Anticipatory method stays within 5pp of Google OR-Tools reactive in the research study | `test_tracks_or_tools_reactive` | verified |
...
| ⏳ | Right delivery list renders cards with van/scooter icons (browser e2e) | `test_delivery_list_and_cards` | unverified |
| ⏳ | Clicking a delivery selects it and opens the inspector (browser e2e) | `test_click_delivery_highlights` | unverified |
| ⏳ | Landing page is on-brand: mascot mark, Poppins display, Cream surface, Pulse Red CTA -> /login (browser e2e) | `test_landing_is_on_brand` | unverified |
| ⏳ | Login page is on-brand (Cream/Pulse Red/Poppins) and rejects bad credentials with an inline error (browser e2e) | `test_login_is_on_brand_and_rejects_bad_creds` | unverified |
